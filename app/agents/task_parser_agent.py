...
class TaskParserAgent:
    ...
    Supports memory context injection: when `memory_context` is set, it
    prepends the context to the system prompt to help LLM understand
    user preferences and scheduling history.
    """
    ...
    def __init__(
        ...
        self.memory_context: str = ""  # 跨会话记忆上下文（方式2注入）
    ...
    def parse(self, instruction: str) -> TaskBatch:
        ...
        base_prompt = PARSE_SYSTEM_PROMPT.replace(
        ...
        # 记忆上下文注入（方式2）：如果有跨会话记忆，前缀到 system prompt
        if self.memory_context:
            ...
            self.memory_context = ""  # 用后即清理，避免泄漏到下一次
        else:
